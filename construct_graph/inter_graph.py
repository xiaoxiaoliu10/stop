...
from torch_geometric.nn import global_max_pool, GATConv, GCNConv
...
class dim_graph_eu(nn.Module):
    def __init__(self, r, pe_ratio, dim_num, pe='lap'):
        super(dim_graph_eu, self).__init__()
        self.k = int(dim_num * r)
        pe_dim = int(pe_ratio * dim_num)
        self.pe = POSENCODINGS[pe](pe_dim)
        self.dim_num = dim_num

    def similarity(self, output):
        output = rearrange(output, 'd b n h->b d (n h)')
        a = torch.cdist(output, output)
        a = -a
        mask = torch.zeros(a.shape[0], a.shape[1], a.shape[2]).to(output.device)
    ...
    def forward(self, output1):
        ...
        adjs = self.similarity(output1)  # [batch_size,d,d]
